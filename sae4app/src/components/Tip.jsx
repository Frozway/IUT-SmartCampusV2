  ...
  return (
    ...
    >
      <div className="flex my-2 items-center">
        ...
        <p className="mx-2">
          {tip.text +
            (shouldCheck ? " Vérifiez que le conseil a bien été appliqué" : "")}
        </p>
        ...
        {isApplied && (
          <p className="text-xs ml-auto">
            Appliqué il y a{" "}
            {new Date().getMinutes() - new Date(tip.appliedAt).getMinutes()}{" "}
            minute(s)
          </p>
        )}
      </div>
      {(!isApplied || shouldCheck) && (
        <div
          className={`${
            shouldCheck
              ? "bg-orange-dark hover:text-orange-dark"
              : "bg-blue-dark hover:text-blue-dark"
          } rounded-lg py-2 px-4 text-white text-center hover:bg-white cursor-pointer`}
